...
def nested_subplots(fig=None, r1=(1, 2), r2=(2, 1), hspace=.2, wspace=.2):
    """
    # input fig is a figure handle, if an existing figure is to be used
...
def swarmplot(df, indeces, ax, vertical=1, spread=3, trend=1, operation=np.mean,
    ...
    try:
        swarmPlot_kw['label']  # swarmplot style
    except:
        swarmPlot_kw['label'] = 'Swarm plot'
    try:
        swarmPlot_kw['m']
    except:
        swarmPlot_kw['m'] = '.'
    ...
    try:
        ...
        swarmPlot_kw['xticks'] = True
    if trend:  # trend line style
        try:
            trendPlot_kw['color']
        except:
            trendPlot_kw['color'] = [.5, .5, .5]
        try:
            trendPlot_kw['style']
        except:
            trendPlot_kw['style'] = '-'
...
def bootstrap_plot(df, indeces, ax, operation=np.mean, nsh=10000, vertical=1,
    ...
    min_bc = [];
    max_bc = []  # mins and max values for y axis lims
    m_b = [];
    ci_b = []  # mean and ci of bootstrapped difference distribution
...
def estimation_plot(input_, indeces=None, vertical=1, EXC=0, trend=1, spread=3, paired=False,
    ...
    - indexes = list of indexes used for multiple control analysis;
    ...
    fig, [m,ci], p-values = figure object and optionally mean, confidence interval
                            and p values of bootstrap estimation (standard and bca corrected)
    ...
    # Swarmplot
    try:
        swarmPlot_kw['s']  # set the marker size here because it's not passed correctly
    except:
    ...
    if nCols == 2:
        xlim = (-.5 / spread * 1.2 * (nCols), nCols - 1 + 1.1 / spread * 1.2 * (nCols))
    else:
        xlim = (-.5 / spread * 1.2 * (nCols / 2), nCols - 1 + 1.1 / spread * 1.2 * (nCols / 2))
    axs[0].set_xlim(xlim)
    axs[1].set_xlim(xlim)
